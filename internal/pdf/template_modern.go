...
func generateModern(m core.Maroto, inv *models.Invoice, qrBytes []byte) {
	modernHeader(m, inv)
	modernParties(m, inv)
	modernItems(m, inv)
	modernTotals(m, inv, qrBytes)
}
...
func modernTotals(m core.Maroto, inv *models.Invoice, qrBytes []byte) {
	hasQR := len(qrBytes) > 0
		...
		m.AddRows(
			row.New(24).WithStyle(&props.Cell{
				...
				col.New(3).Add(image.NewFromBytes(qrBytes, extension.Png,
					props.Rect{Percent: 90, Center: true},
				)),
